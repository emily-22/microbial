...
lost_AFDM_day<-(((spongedf$Pre.AFDM ) - ( spongedf$Post.AFDM))* (.9/.3) )/ (spongedf$Days.in.Stream)

plot(lost_AFDM_day~g_carbon_AFDM_resp)
abline(a=0,b=1)
abline(lm(lost_AFDM_day~g_carbon_AFDM_resp), lwd=3)
summary(lm(lost_AFDM_day~g_carbon_AFDM_resp))


plot(lost_AFDM_day~spongedf$Chloride)
plot(g_carbon_AFDM_resp~ spongedf$Chloride)
plot(lost_AFDM_day~spongedf$SRP)
plot(g_carbon_AFDM_resp~ spongedf$SRP)
plot(lost_AFDM_day~spongedf$Ammonium)
plot(g_carbon_AFDM_resp~ spongedf$Ammonium)
plot(lost_AFDM_day~spongedf$DIN)
plot(g_carbon_AFDM_resp~ spongedf$DIN)
```
...
# just wood
plot(WC_lost_day~Wg_carbon_resp, xlim=c(0,.0005), ylim=c(-0.002,0.001), pch=19)
abline(a=0,b=1)
...
W_lost_AFDM_day<-(((wooddf4$Pre.AFDM ) - ( wooddf4$Post.AFDM)) * (.99/.45))/ (wooddf4$Days.in.Stream)


# just wood
plot(W_lost_AFDM_day~Wg_carbon_AFDM_resp,)# pch=19, xlim=c(0,.002), ylim=c(-0.002, 0.0025))
abline(lm((W_lost_AFDM_day~Wg_carbon_AFDM_resp)), lwd=3)
abline(a=0,b=1)

# Wood and cellulose
plot(W_lost_AFDM_day~Wg_carbon_AFDM_resp, pch=19, xlim=c(0,.06), ylim=c(-0.01, 0.04))
points(lost_AFDM_day~g_carbon_AFDM_resp)
abline(a=0,b=1)

abline(lm(W_lost_AFDM_day~Wg_carbon_AFDM_resp), lty=3)
abline(lm(lost_AFDM_day~g_carbon_AFDM_resp), lty=3)
